...
export default function DashboardPage() {
  ...
  return (
    <ConsoleLayout title="Dashboard" roleLabel="Restaurant" nav={RESTAURANT_NAV}>
      {error && (
      ...
      <div className="nabin-stack">
        <section className="nabin-card">
          <div className="nabin-card__header">
            <div>
              ...
              <p className="nabin-cell-meta">
                {humanise(merchant.merchant_type) || 'Restaurant'} &middot; {merchant.address ?? 'Address on file'}
              </p>
            </div>
            <span
          ...
          <div className="nabin-row" style={{ flexWrap: 'wrap' }}>
            ...
            {merchant.fssai_license && <span className="nabin-chip">FSSAI {merchant.fssai_license}</span>}
          </div>
        </section>

        <div className="nabin-grid">
          <div className="nabin-card">
            <p className="nabin-stat__label">Open food orders</p>
            <p className="nabin-stat__value nabin-num">{data?.activeOrdersCount ?? '—'}</p>
          </div>
          <div className="nabin-card">
            <p className="nabin-stat__label">Order value on record</p>
            <p className="nabin-stat__value nabin-num">{inr(data?.todaySales)}</p>
          </div>
          <div className="nabin-card">
            <p className="nabin-stat__label">Food orders, all time</p>
            <p className="nabin-stat__value nabin-num">{data?.totalOrders ?? '—'}</p>
          </div>
        </div>

        <p className="nabin-notice">
          Totals cover every food order stored for this merchant. The backend has no date filter on
          this endpoint yet, so this is not a calendar-day figure.
        </p>
        ...
        <div className="nabin-grid nabin-grid--split">
          <section className="nabin-card">
            ...
            {pending && !data ? (
              ...
              <p className="nabin-cell-meta">No food orders recorded for this store yet.</p>
            )}
          </section>
