...
class NabinDatabase {
  ...
  getLedgerEntries(filters = {}) {
    ...
    if (filters.transactionId) {
      list = list.filter(e => e.transactionId === filters.transactionId || e.referenceId === filters.transactionId || e.id === filters.transactionId || e.entryId === filters.transactionId);
    }
    return list;
  }
  ...
  async recordPaymentWebhook({ eventId, eventType, paymentId, amount, status, signature, payload, rawBody = null }) {
    if (this.paymentRepo) {
      return await this.paymentRepo.recordPaymentWebhook({ eventId, eventType, paymentId, amount, status, signature, payload, rawBody });
    }
    if (!eventId) {
    ...
    this.processedWebhookIds.add(eventId);
    return { success: true, duplicate: false, paymentId };
  }
  ...
  async createPaymentSession({ customerId, amount, currency = 'INR', serviceType = 'RIDE', jobId = null, metadata = {} }) {
    if (this.paymentRepo) {
      return await this.paymentRepo.createPaymentSession({ customerId, amount, currency, serviceType, jobId, metadata });
    }
    if (!amount || amount <= 0) {
    ...
    const session = { orderId, customerId: customerId || 'usr_cust_anon', amount: Number(amount), currency, serviceType, jobId, status: 'PAYMENT_PENDING' };
  ...
  async verifyPaymentSession({ orderId, paymentId, signature, customerId = null, status = 'SUCCESS', failureReason = null }) {
    if (this.paymentRepo) {
      return await this.paymentRepo.verifyPaymentSession({ orderId, paymentId, signature, customerId, status, failureReason });
    }
    if (!this.paymentSessions) this.paymentSessions = new Map();
    const session = this.paymentSessions.get(orderId);
    if (!session) throw new Error(`Payment session [${orderId}] not found in database.`);
